...
class firnAirModel:
    ...
    def FirnAir_SS(self, gaschoice): 
        ...
        z_res = self.cc['z_resolution'] #resolution of grid, m
        ...
        self.rhoHL = MPRHO.rhoHLAnalytic(self.R,T,self.self.rho_i,rho0,self.rho_bco,z_nodes,Accu_m) # Get density profile from H&L analytic
        ...
        diffu, d_eddy, diffu_full_fre, diffu_full_sch, diffu_full_sev, diffu_full_data = self.diffusivity(self.cc, rho_co, por_co, por_tot, por_cl, por_op, z_co, czd, LIZ, d_0, D_x, p_a, z_nodes, T, sitechoice, self.rhoHL) #get diffusivity profiles
    ...
    def FirnAir_TR(self, cc, gaschoice,jj):
        ...
        ConcPath = os.path.join(self.dataSave, FID) #Save location.
    ...
    def firnair(self):
        ...
        for jj in xrange(nogas):
            ...
            print jj, gaschoice      
            runtype = self.cc["runtype"] #this line chooses transient or steady-state        
            if runtype == 'transient':
                phi, diffu_hold, rho_hold, z_nodes = self.FirnAir_TR(self.cc, gaschoice, jj)
